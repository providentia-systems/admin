  ...
  final session = SessionController(
    api: FakeApi(
      (_) async => jsonResponse({
        ...
        'profile': {
          'administratorAccess': {
            'features': {for (final p in permissions) p: true},
          },
        },
      }),
    ),
...
void main() {
  test('group listing is scoped and updates carry their revision', () async {
    ...
    final repository = AccessRepository(api);
    ...
    await repository.save({
      ...
      'name': 'Starter',
      'description': '',
      'features': {'members.invite': false},
      'limits': {'members.total': 3},
      'delegablePermissions': [],
      'rolePermissions': {},
      'revision': 7,
    });
    expect(api.requests.last.method, 'PUT');
    expect((api.requests.last.body as Map)['expectedRevision'], 7);
  });
  testWidgets(
    'approval selects an unprotected group and submits both revisions',
    (tester) async {
      ...
      final api = FakeApi((request) async {
        if (request.path.endsWith('/groups')) {
          return jsonResponse({
            'data': [
              {'id': 'root', 'name': 'System owner', 'protected': true},
              {'id': 'reviewers', 'name': 'Reviewers', 'protected': false},
            ],
          });
        }
        return jsonResponse({
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: PlatformAdministratorsPage(api: api, session: session),
          ),
        ),
      );
      ...
      await tester.pumpAndSettle();
      expect(find.text('System owner'), findsNothing);
      await tester.tap(find.text('Reviewers'));
      await tester.pumpAndSettle();
      final review = api.requests.singleWhere((r) => r.method == 'POST');
      expect(review.path, '/api/v1/admin/administrators/candidate/review');
      expect(review.body, {
        'status': 'approved',
        'groupId': 'reviewers',
        'expectedRevision': 3,
  ...
  testWidgets(
    'listing permission cannot approve or change administrator groups',
    (tester) async {
      final session = await sessionFor(['administrators.read']);
      addTearDown(session.dispose);
      final api = FakeApi(
        (_) async => jsonResponse({
          'data': [
